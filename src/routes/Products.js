...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await axios.post("https://outrageous-shoulder-pads-fly.cyclic.app/products/create", formData);
      ...
      <Grid templateColumns={{ base: "1fr", md: "repeat(2, 1fr)" }} gap={4}>
        {filteredData.map((gemstone, index) => (
          <Container key={index} maxW="5xl" p={{ base: 5, md: 6 }}>
            <Stack bgColor="#f8f9fa" maxW="100%" spacing={2} p={4} rounded="md">
              ...
              <Accordion allowToggle>
                <AccordionItem>
                  ...
                  <AccordionPanel pb={4}>{gemstone.benefits}</AccordionPanel>
                </AccordionItem>
