...
const renderComp = () => {
  ...
  const fetchAdd = async () => {
    try {
      const response = await axios.get(
        "https://dukhbhanjan.onrender.com/address"
      );
  ...
  const handleEditOrderStatus = async(id, status) => {
    ...
    if(status=== "dispatch"){
      const response =  await axios.patch(`https://dukhbhanjan.onrender.com/order/update/${id}`, {status:"dispatch"})
      try {
      ...
      try {
        const response =  await axios.patch(`https://dukhbhanjan.onrender.com/order/update/${id}`, {status:"delivered"})
        if(response.data.state){
          toast({
            title: response.data.msg,
            status: 'success',
